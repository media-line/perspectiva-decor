 ...
 */
	...
	$placeholder = !$isCurrentPreset ? "MAIN_UI_FILTER__PLACEHOLDER_DEFAULT" : "MAIN_UI_FILTER__PLACEHOLDER_WITH_FILTER";

	if ($arResult["DISABLE_SEARCH"])
	{
...
<!-- Final :: Search -->
<div class="main-ui-filter-search<?=$arResult["DISABLE_SEARCH"] ? " main-ui-filter-no-search" : ""?>" id="<?=$arParams["FILTER_ID"]?>_search_container">
	<input
			type="text"
			tabindex="1"
			autofocus=""
			...
			autocomplete="off">
	<div class="main-ui-item-icon-block<?=(!empty($arResult["CURRENT_PRESET"]["FIND"]) || $isCurrentPreset) ? " main-ui-show" : ""?>">
...
<script type="text/html" id="<?=$arParams["FILTER_ID"]?>_GENERAL_template">
	<div class="main-ui-filter-wrapper <?=$arParams["VALUE_REQUIRED_MODE"] == true ? " main-ui-filter-value-required-mode" : ""?>">
				...
				<div class="main-ui-filter-sidebar-item-container">
					<? if (is_array($arResult["PRESETS"])) : ?>
						<? foreach ($arResult["PRESETS"] as $key => $preset) : ?>
							...
							?><?=$preset["IS_PINNED"] ? " main-ui-item-pin" : ""?>" data-id="<?=$preset["ID"]?>"<?
							?><?=$preset["IS_PINNED"] ? " title=\"".Loc::getMessage("MAIN_UI_FILTER__IS_SET_AS_DEFAULT_PRESET")."\"" : " "?>>
				...
				<div class="main-ui-filter-field-button-container">
					<div class="main-ui-filter-field-button-inner">
						...
						<? if ($USER->CanDoOperation("edit_other_settings")) : ?>
							<span class="webform-small-button webform-small-button-decline main-ui-filter-field-button main-ui-filter-save-for-all">
								<span class="main-ui-filter-field-button-item"><?=Loc::getMessage("MAIN_UI_FILTER__CONFIRM_APPLY_FOR_ALL")?></span>
							</span>
						<? endif; ?>
