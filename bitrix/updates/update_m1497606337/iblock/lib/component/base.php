...
abstract class Base extends \CBitrixComponent
{
	...
	protected function getRandomRecommendation($ids)
	{
		...
		$filterIds = $ids + $this->arParams['FILTER_IDS'];
		if (!empty($filterIds))
		{
			$filter['!ID'] = $filterIds;
		}
...
//			'CondIBActive' => 'ACTIVE',
	...
	protected function calculatePriceBlock(array $product, array $priceBlock, $ratio, $defaultBlock = false)
	{
		...
		$resultWithVat = (!isset($this->arParams['PRICE_VAT_INCLUDE']) || $this->arParams['PRICE_VAT_INCLUDE'] !== 'N');
		...
		foreach ($priceBlock as $rawPrice)
		{
			...
			if ($discountPrice !== false)
			{
				...
				$roundPrice = $roundPriceWithVat / $percentPriceWithVat;
		...
		if ($this->arResult['MODULES']['catalog'])
		{
			...
			switch ($item['PRODUCT']['TYPE'])
			{
				case Catalog\ProductTable::TYPE_SET:
					$item['OFFERS'] = array();
					$item['CHECK_QUANTITY'] = $item['PRODUCT']['QUANTITY_TRACE'] === 'Y' && $item['PRODUCT']['CAN_BUY_ZERO'] === 'N';
					break;
