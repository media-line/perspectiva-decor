...
class DiscountManager
{
	...
	{
		$cachedPropertyValues = $propertyValues = array_fill_keys($productIds, array());
		foreach ($entityData['needProperties'] as $iblock => $propertyList)
		{
			...
			$needToLoad = array_fill_keys($productIds, self::$productProperties? false : true);
			if(self::$productProperties)
			{
				foreach($propertyList as $i => $prop)
				{
					foreach($productIds as $productId)
					{
						if(!in_array($productId, $iblockData['iblockElement'][$iblock]))
						{
							continue;
						}
						...
						if($propData)
						{
							$cachedPropertyValues[$productId][$propData['CODE']] = $propData;
						}
						else
						{
							$needToLoad[$productId] = true;
						}
					}
				}
			}

			$needToLoad = array_filter($needToLoad);
			if($needToLoad)
			{
				$propertyValues = array_fill_keys(array_keys($needToLoad), array());
		...
		foreach($cachedPropertyValues as $productId => $props)
		{
			if(!empty($propertyValues[$productId]))
			{
				$propertyValues[$productId] = array_merge($propertyValues[$productId], $props);
			}
			else
			{
				$propertyValues[$productId] = $props;
			}
		}
